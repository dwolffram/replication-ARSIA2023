...
# Visualize forecasts ------------------------------------------------------------------------
df <- filter(
  my_pred, target_end_date >= "2013-01-01 12:00:00",
...
forecast_plot <- ggplot(mapping = aes(x = target_end_date)) +
  ...
  scale_color_manual(
    name = NULL, values = c("Truth" = "darkred", "Median" = "deepskyblue3"),
    guide = guide_legend(order = 1, direction = "horizontal")
  ) +
  scale_alpha_manual(
    name = "Prediction interval", values = rel_alpha,
    guide = guide_legend(
      order = 2, override.aes = list(alpha = abs_alpha),
...
ggsave("figures/10_GEFCom14_CaseStudy.pdf",
  plot = fig10, width = 160, height = 200,
