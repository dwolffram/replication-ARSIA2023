...
reldiag_Engel = data_long %>%
  ...
  group_by(model,type) %>%
  do(reldiag(.$value, .$truth, alpha = quantile, n_resamples = n_resamples, resample_log =TRUE, digits = digits)) %>% 
...
scores = reldiag_Engel %>%
  ...
  mutate(label = paste0(c("\nuMCB ","cMCB ","DSC ","UNC "),
                        ...
                        collapse = "\n"))
score_layer = list(
  geom_label(data = scores, mapping = aes(x = 0, y = Inf, label = sprintf(paste0("bar(S)~'%0.",digits,"f'"), score)),
             size = 6 * 0.36, hjust = 0, vjust = 1, label.size = NA, alpha = 0, label.padding = unit(1, "lines"), parse = TRUE),
  geom_label(data = scores, mapping = aes(x = 0, y = Inf, label = label),
             size = 6 * 0.36, hjust = 0, vjust = 1, label.size = NA, alpha = 0, label.padding = unit(1, "lines"), parse = FALSE)
)

reliability_plot = reldiag_Engel %>% 
  group_by(model,type) %>%
  plot.reldiag(score_decomp = FALSE) +
...
scores = data_long %>%
  ...
  do(reldiag(.$value, .$truth, alpha = .$quantile[1],resampling = FALSE,digits = digits)) %>%
...
table[c("quantile", "variable","Linear_In-sample","Log-linear_In-sample","Isotonic_In-sample",
